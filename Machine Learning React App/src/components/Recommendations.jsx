...
const Recommendations = ({ data }) => {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-start space-x-4">
          ...
          <div className="flex-grow">
            ...
            <div className="text-3xl font-bold text-green-700 mb-2">
              {resultsSummary.best_precision_model || best.model || 'N/A'}
            </div>
            <p className="text-gray-700 mb-4">
              Best performing model with highest precision ({((resultsSummary.best_precision_score || best.precision) * 100).toFixed(2)}%). 
              Delivers optimal balance of accuracy, recall, and interpretability for heart disease prediction.
            </p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="bg-white rounded p-3 shadow-sm">
                <div className="text-2xl font-bold text-green-600">
                  {((resultsSummary.best_precision_score || best.precision) * 100).toFixed(1)}%
                </div>
                <div className="text-xs text-gray-600">Precision</div>
              </div>
              <div className="bg-white rounded p-3 shadow-sm">
                <div className="text-2xl font-bold text-blue-600">
                  {((resultsSummary.best_recall_score || best.recall) * 100).toFixed(1)}%
                </div>
                <div className="text-xs text-gray-600">Recall</div>
              </div>
              <div className="bg-white rounded p-3 shadow-sm">
                <div className="text-2xl font-bold text-purple-600">
                  {((resultsSummary.best_f1_score || best.f1) * 100).toFixed(1)}%
                </div>
                <div className="text-xs text-gray-600">F1 Score</div>
              </div>
              <div className="bg-white rounded p-3 shadow-sm">
                <div className="text-2xl font-bold text-orange-600">
                  {((resultsSummary.best_roc_auc_score || best.roc_auc) * 100).toFixed(1)}%
                </div>
      ...
      {resultsSummary.precision_target && (
        <div className={`card ${
          resultsSummary.models_meeting_target > 0 
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div>
              ...
              <div className="text-2xl font-bold text-blue-600">
                {(resultsSummary.precision_target * 100).toFixed(0)}%
              </div>
            </div>
            <div>
              <div className="text-gray-600">Best Achieved</div>
              <div className="text-2xl font-bold text-green-600">
                {((resultsSummary.best_precision_score || 0) * 100).toFixed(1)}%
              </div>
            </div>
            <div>
              <div className="text-gray-600">Models Meeting Target</div>
              <div className={`text-2xl font-bold ${
                resultsSummary.models_meeting_target > 0 ? 'text-green-600' : 'text-yellow-600'
              }`}>
                {resultsSummary.models_meeting_target || 0}
              </div>
            </div>
      ...
      {/* Next Steps */}
      {resultsSummary.next_steps && resultsSummary.next_steps.length > 0 && (
        <div className="card bg-blue-50 border-l-4 border-l-blue-600">
          ...
          <ol className="space-y-3">
            {resultsSummary.next_steps.map((step, idx) => (
      ...
      {allModels.length > 1 && (
        <div className="card">
          ...
          <div className="space-y-3">
            {allModels.slice(1, 6).map((model, idx) => (
